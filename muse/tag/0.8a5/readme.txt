...
Muse API is now developed under JDK 1.4.  But it should work properly
under JDK 1.2 and above.
...
A default ant.sh and ant.bat script is provided that allows you to actually run
the compiles and ant tasks without installing a separate ANT distribution.  Muse
can is now totally self-sufficient.  You can simply run the script (ie. ant jar)
and it will utilize the ANT libraries that comes with Muse.
...
You now have the option to jar up the specific modules that you want.  For just
the necessary files for a specific module, you would simply specify "ant jar-{module name}"
and it will jar up only the files that are needed for that module.  Some examples are,

ant jar-jabber
ant jar-gnutella

You can jar up the examples as well using "ant jar-examples".  muse-examples.jar is already
created and installed inside the lib/ directory for you to use to run the examples
directly out of the distribution.

Even though the Muse modules can be separated, remember that the required libraries are still
needed.  However, not all libraries are required for individual modules.  You
will have to find out which one requires which library.
...
Echomine Muse (c) 2001, 2002, 2003 Echomine.  All right reserved.